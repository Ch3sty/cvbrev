...
/**
 * Forskare - premium CV-mall fOr akademiska forskare och doktorander.
 *
 * Designprinciper:
 *  - Centrerad header med namn + titel + institution
 *  - 65/35 layout: forskningserfarenhet vanster + publikationer/konferenser hoger
 *  - Section: "Forskningserfarenhet" istallet for arbetslivserfarenhet
 *  - Auto-genererad sektion "Senaste publikationer" fran cvData.projects
 *  - Subtil indigo-accent (#3730a3) - akademisk auktoritet
 *  - Source Serif Pro body (akademisk standard)
 *  - Inga foton (akademiska CV:n ar meriter-drivna)
 *  - ATS-saker
 */
function generateForskareHTML(cvData: CVMetadata, options: any = {}): string {
        ...
        <div class="cv-container">
            ...
            <div class="body-grid">
                <div class="main-col">
                    ...
                    <div class="section">
                        <h2 class="section-heading">Forskningserfarenhet</h2>
                        ${cvData.experience
                            .sort((a, b) => {
                ...
                <div class="side-col">
                    ${projekt.length > 0 ? `
                    <div class="section">
                        <h3 class="side-heading">Senaste publikationer</h3>
                        ${projekt.slice(0, 5).map(p => `
                        <div class="pub-row">
                            <div class="pub-name">${p.name || p.title || ''}</div>
                            ${p.description ? `<div class="pub-meta">${p.description}</div>` : ''}
                        </div>`).join('')}
                    </div>` : ''}

                    ${certifikat.length > 0 ? `
                    <div class="section">
                        <h3 class="side-heading">Stipendier & utmärkelser</h3>
                        ${certifikat.map(cert => `
                        <div class="cert-row">
                    ...
                    ${allSkills.length > 0 ? `
                    <div class="section">
                        <h3 class="side-heading">Forskningsområden</h3>
...
export const forskareTemplate: CVTemplateGenerator = {
  templateId: 'forskare',
