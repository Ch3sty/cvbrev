...
export default function InterviewTipsCard({ scores }: { scores: BigFiveScores }) {
  ...
  return (
    <motion.section
      ...
      style={{ boxShadow: '0 8px 24px -12px rgba(99, 102, 241, 0.18)' }}
    >
      <div className="p-5 sm:p-7">
        ...
        <ul className="space-y-2">
          {tips.map((tip, i) => (
            ...
            >
              <span className="flex-shrink-0 w-5 h-5 rounded-full bg-indigo-100 text-indigo-700 text-xs font-bold flex items-center justify-center mt-0.5 tabular-nums">
                {i + 1}
              </span>
              <span>{tip}</span>
            </motion.li>
