      ...
      {options.map((option, idx) => {
        ...
        return (
          <motion.button
            ...
            className={`w-full text-left flex items-center gap-3 sm:gap-4 px-4 sm:px-5 py-3.5 sm:py-4 rounded-xl border-2 transition-colors min-h-[60px] touch-manipulation ${
              isSelected
          ...
          >
            ...
            <div
              className={`flex-shrink-0 w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center font-bold text-sm sm:text-base transition-all ${
                isSelected ? 'bg-orange-600 text-white' : 'bg-orange-50 text-orange-700 border border-orange-200'
              }`}
            >
              {isSelected ? (
            ...
            <span
              className={`flex-1 text-sm sm:text-base font-medium leading-snug ${
                isSelected ? 'text-orange-900 font-semibold' : 'text-neutral-700'
              }`}
            >
              {option.text}
            </span>
