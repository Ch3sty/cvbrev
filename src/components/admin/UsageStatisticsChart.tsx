...
interface UsageStatisticsChartProps {
  data: UsageStats[];
  metric?: 'calls' | 'users' | 'avgCalls' | 'successRate';
}
...
export default function UsageStatisticsChart({ data, metric = 'calls' }: UsageStatisticsChartProps) {
  const chartData = data.map((item) => ({
    ...
    value: metric === 'calls'
      ...
      : metric === 'users'
        ? item.uniqueUsers
        : metric === 'avgCalls'
          ? Math.round(item.avgCallsPerUser * 10) / 10
          : item.successRate,
  ...
  const getLabel = () => {
    switch (metric) {
      case 'calls':
        return 'Totalt antal anrop';
      case 'users':
        return 'Unika användare';
      case 'avgCalls':
        return 'Snitt anrop/användare';
      case 'successRate':
        return 'Success rate (%)';
      default:
        return 'Värde';
