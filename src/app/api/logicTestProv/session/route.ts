...
export async function POST() {
  try {
    ...
    if (createError) {
      console.error('Error creating prov session:', createError);
...
// GET: lista användarens prov-sessioner (för resultat + statistik).
export async function GET(request: Request) {
  try {
    ...
    if (fetchError) {
      console.error('Error fetching prov sessions:', fetchError);
