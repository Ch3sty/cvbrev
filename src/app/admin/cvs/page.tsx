...
export default function AdminCVsPage() {
  ...
  return (
    <div className="space-y-6 p-4 md:p-6 lg:p-8 bg-navy-900 min-h-screen">
      {/* Page header */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white">Hantera CV:n</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-navy-800 rounded-lg p-4 border border-gray-700">
          <p className="text-xs text-gray-400">Totalt antal CV:n</p>
          <h3 className="text-xl font-bold text-white mt-1">{stats.totalCVs}</h3>
        </div>
        
        <div className="bg-navy-800 rounded-lg p-4 border border-gray-700">
          <p className="text-xs text-gray-400">Genomsnittlig längd</p>
          <h3 className="text-xl font-bold text-white mt-1">{stats.averageLength.toLocaleString()} tecken</h3>
        </div>
        
        <div className="bg-navy-800 rounded-lg p-4 border border-gray-700">
          <p className="text-xs text-gray-400">Idag</p>
          <h3 className="text-xl font-bold text-white mt-1">{stats.todaysCVs}</h3>
        </div>
        
        <div className="bg-navy-800 rounded-lg p-4 border border-gray-700">
          <p className="text-xs text-gray-400">Denna vecka</p>
          <h3 className="text-xl font-bold text-white mt-1">{stats.thisWeekCVs}</h3>
        </div>
      </div>
        ...
        {isLoading ? (
          <div className="flex justify-center items-center h-64">
            <div className="flex flex-col items-center">
              ...
              <p className="text-gray-400">Laddar CV:n...</p>
            </div>
          </div>
        ) : (
          <>
              ...
              <table className="min-w-full divide-y divide-gray-700">
                <thead className="bg-navy-700/50">
                  <tr>
                    ...
                    <th 
                      scope="col"
                      className="hidden lg:table-cell px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-navy-600/50 transition-colors"
                ...
                <tbody className="bg-navy-800 divide-y divide-gray-700">
                  {filteredCVs.length === 0 ? (
                     <tr>
                       <td colSpan={6} className="px-6 py-12 text-center">
                         <div className="flex flex-col items-center">
                          ...
                          <h3 className="text-lg font-semibold text-white mb-1">Inga CV:n hittades</h3>
                    ...
                    getCurrentPageCVs().map((cv) => (
                      <tr key={cv.id} className="hover:bg-navy-700/60 transition-colors group">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            <div className="flex-shrink-0 h-10 w-10 rounded-full bg-pink-600/20 flex items-center justify-center ring-1 ring-pink-500/30">
                              <File className="w-5 h-5 text-pink-400" />
                            </div>
                            <div className="ml-4">
                              <div className="text-sm font-medium text-white truncate max-w-[200px]" title={cv.file_name}>
                                {cv.file_name}
                              </div>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            <div className="flex-shrink-0 h-8 w-8 rounded-full bg-navy-600 flex items-center justify-center ring-1 ring-navy-500">
                            ...
                            <div className="ml-3">
                              <div className="text-sm font-medium text-white truncate max-w-[150px]" title={cv.full_name || 'Namnlös användare'}>
                                {cv.full_name || <span className="italic text-gray-400">Namnlös</span>}
                              </div>
                              <div className="text-xs text-gray-400 truncate max-w-[150px]" title={cv.email}>{cv.email}</div>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <div className="flex items-center justify-end gap-2">
                            <button 
                              ...
                              className="text-blue-400 hover:text-blue-300 p-1 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-navy-800"
                   ...
                   <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between md:justify-end">
                     <div>
                      <p className="text-sm text-gray-400">
                        ...
                        {' '}till{' '} 
                        <span className="font-medium text-white">{Math.min(currentPage * pageSize, filteredCVs.length)}</span>
                        {' '}av{' '}
                        <span className="font-medium text-white">{filteredCVs.length}</span> resultat
                      </p>
                    </div>
                    <div className="ml-4">
      ...
      {showPreviewModal && selectedCV && (
        <div 
           className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75 transition-opacity p-4" 
        ...
        >
          <div 
             className="bg-navy-800 rounded-lg w-full max-w-4xl max-h-[90vh] relative shadow-xl border border-navy-700 flex flex-col"
             onClick={(e) => e.stopPropagation()}
          >
            {/* Modal Header */}
            <div className="border-b border-gray-700 px-6 py-4 flex items-center justify-between flex-shrink-0">
              <div>
                <h3 id="modal-title" className="text-xl font-semibold text-white">{selectedCV.file_name}</h3>
            ...
            <div className="flex-1 overflow-auto">
              ...
              <div className="px-6 py-4 border-b border-gray-700/50">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                  <div>
                    ...
                    <span className="ml-2 text-white font-medium">{formatFileSize(selectedCV.cv_text)}</span>
                  </div>
                  <div>
                    <span className="text-gray-400">Tecken:</span>
                    <span className="ml-2 text-white font-medium">{selectedCV.cv_text.length.toLocaleString()}</span>
                  </div>
                  <div>
                    <span className="text-gray-400">Skapad:</span>
                    <span className="ml-2 text-white font-medium">{formatDate(selectedCV.created_at)}</span>
                  </div>
                </div>
              </div>
              ...
              {/* CV Content */}
              <div className="p-6">
                <div className="bg-white text-gray-800 rounded-md p-6 overflow-auto prose max-w-none" style={{ maxHeight: '400px' }}>
                  {selectedCV.cv_text ? (
            ...
            {/* Modal Footer */}
            <div className="border-t border-gray-700 px-6 py-4 flex justify-end gap-2 flex-shrink-0">
              <button
                onClick={closePreviewModal}
