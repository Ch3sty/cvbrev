...
export default function JobbcoachenClient({
  initialCvCount,
  initialLetterCount,
}: {
  /** Serverräknade dokument. Välkomstkortets text och knappen hänger på dem,
      så de måste stå rätt i första HTML. */
  initialCvCount: number;
  ...
  const [cvCount, setCvCount] = useState(initialCvCount);
  const [letterCount, setLetterCount] = useState(initialLetterCount);
  ...
  // Dokumentantalen kommer serverrenderade. Förut låg de i en effekt som
  // först gjorde auth.getUser() över nätet och sedan två count-frågor i tur
  // och ordning, alltså tre seriella rundturer. Välkomstkortet, som är
