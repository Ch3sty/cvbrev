    ...
    <div className="space-y-4">
      {questions.map((q, index) => {
        ...
        return (
          <div
            key={q.question}
            className="rounded-xl border border-indigo-100 bg-white p-4 sm:p-5"
          >
            ...
            <div className="mt-3 rounded-xl bg-indigo-50/60 border border-indigo-100 p-3">
              <p className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-[0.1em] text-indigo-700 mb-1.5">
              ...
              <ul className="space-y-1">
                {q.listenFor.map((item) => (
                  <li
                    key={item}
                    className="flex items-start gap-2 text-xs text-indigo-900/80 leading-relaxed"
                  >
                    <span
            ...
            <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              {STAR_FIELDS.map((field) => (
                <label key={field.key} className="block">
                  <span className="block text-xs font-bold uppercase tracking-[0.1em] text-neutral-500 mb-1">
                    {field.label}
                  </span>
                  <textarea
                    ...
                    rows={3}
                    className="w-full rounded-xl border border-neutral-200 bg-white px-3 py-2.5 text-[13px] text-neutral-800 leading-relaxed placeholder:text-neutral-300 focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 resize-y"
                  />
                </label>
              ))}
            </div>
