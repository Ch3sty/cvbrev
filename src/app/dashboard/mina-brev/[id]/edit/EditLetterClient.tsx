...
export default function EditLetterClient({
  id,
  initialLetter,
}: {
  id: string;
  /** Serverhämtat brev. Är det satt är formuläret ifyllt redan i första
      HTML och klienten gör ingen rundtur före första målningen. */
  initialLetter: Letter | null;
}) {
  const router = useRouter();
  // Redigeringen läser aldrig brevlistan; utan flaggan hämtades den ändå.
  const {
    getLetter,
    currentLetter: hamtatBrev,
    isLoading,
    error,
    editLetter,
  } = useLetters({ skipInitialFetch: true });
  const { successWithMascot } = useNotification();

  const currentLetter = hamtatBrev && hamtatBrev.id === id ? hamtatBrev : initialLetter;

  const [formData, setFormData] = useState({
    title: initialLetter?.title || '',
    company: initialLetter?.company || '',
    job_title: initialLetter?.job_title || '',
  ...
  const [harForsokt, setHarForsokt] = useState(Boolean(initialLetter));
  /** Formuläret fylls en gång. Utan den här spärren skrev en sen
      klienthämtning över det användaren just skrivit. */
  const formFilledRef = useRef(Boolean(initialLetter));

  useEffect(() => {
    // Servern hann före. Ingen klienthämtning behövs.
    if (initialLetter) return;
    if (id && !initialLoadRef.current) {
      initialLoadRef.current = true;
      Promise.resolve(getLetter(id)).finally(() => setHarForsokt(true));
    }
  }, [id, getLetter, initialLetter]);

  useEffect(() => {
    if (currentLetter && !formFilledRef.current) {
  ...
  if (!currentLetter && (!harForsokt || isLoading)) {
    return (
