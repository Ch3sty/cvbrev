...
import { AvanceradHeroIllustration } from '../../matrislogik-avancerad/components/illustrations/AvanceradIcons';

interface ExpertTestHeroProps {
  bestScore?: number;
  bestPercentage?: number;
}

export default function ExpertTestHero({ bestScore, bestPercentage }: ExpertTestHeroProps) {
  return (
    <motion.section
      ...
      style={{
        background: 'linear-gradient(135deg, #EA580C 0%, #B91C1C 45%, #7F1D1D 100%)',
        boxShadow: '0 20px 60px -20px rgba(185, 28, 28, 0.5)',
      }}
    >
      <svg className="absolute inset-0 w-full h-full opacity-25 pointer-events-none" aria-hidden="true">
        <pattern id="hero-exp-dots" x="0" y="0" width="28" height="28" patternUnits="userSpaceOnUse">
          <circle cx="14" cy="14" r="1" fill="white" />
        </pattern>
        <rect width="100%" height="100%" fill="url(#hero-exp-dots)" />
      </svg>
      ...
      <div className="relative p-6 sm:p-8 md:p-10 lg:p-12">
        <div className="max-w-2xl">
          ...
          <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold leading-tight tracking-tight mb-3">
            Matrislogik på expertnivå
          </h1>
          <p className="text-sm sm:text-base md:text-lg opacity-95 leading-relaxed mb-6 max-w-xl">
            Vår svåraste nivå. Mönster som roterar och förflyttas i flera steg. Här räcker det inte att gissa.
          </p>
          ...
          <div className="flex flex-wrap items-center gap-x-5 gap-y-2 pt-5 border-t border-white/20">
            ...
            <Divider />
            <Stat icon={<Clock className="w-3.5 h-3.5" strokeWidth={2.5} />} label="ca 30 min" />
            <Divider />
            <Stat icon={<Flame className="w-3.5 h-3.5" strokeWidth={2.5} />} label="Svåraste nivån" />
