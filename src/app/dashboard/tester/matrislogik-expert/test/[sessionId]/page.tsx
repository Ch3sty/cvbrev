...
import { selectExpertQuestionsForSession, QUESTIONS_PER_SESSION } from '@/lib/logicTestV7/selectQuestions.v7';
...
export default function TestSessionPage({ params }: PageProps) {
  ...
  const questions = useMemo(
    () => (sessionId ? selectExpertQuestionsForSession(sessionId) : []),
    [sessionId]
  );
  ...
  const handleFinishTest = async () => {
    ...
    try {
      ...
      if (data.success || data.message) {
        router.push(`/dashboard/tester/matrislogik-expert/test/${sessionId}/results`);
      }
    } catch (error) {
