...
import { useAnswerShortcuts } from '@/lib/logic/useAnswerShortcuts';
import type { ClientQuestion, UserAnswer } from '@/lib/logic/types';
import { ChevronLeft, ChevronRight, Send } from 'lucide-react';

export default function MatrislogikTestPage() {
  ...
  useEffect(() => {
    async function loadSession() {
      try {
        const response = await fetch('/api/logic/validate-session', {
          method: 'POST',
        ...
        if (!response.ok) {
          router.push('/dashboard/tester/matrislogik?error=session-error');
          return;
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/logic/submit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      router.push(`/dashboard/tester/matrislogik/results/${result.attemptId}`);
    } catch (error) {
