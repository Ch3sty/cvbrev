      ...
      <div className="grid grid-cols-3 gap-1 mb-8 justify-center">
        {grid.flatMap((row, r) =>
          row.map((cell, c) => (
            <div
              key={`${r}-${c}`}
              className="w-[110px] h-[110px] flex items-center justify-center border-2 border-blue-300 rounded-lg bg-white"
            >
              {cell ? (
                <SvgCell cell={cell} showGrid={showGrid} />
              ) : (
                <div className="text-6xl text-blue-400 select-none">?</div>
              )}
            </div>
          ))
        )}
