...
import {
  ...
  TOTAL_QUESTIONS,
} from '@/lib/verbalTestExpert/selectPassages';
import {
  getScoreByDifficulty,
  getScoreByType,
} from '@/lib/verbalTestExpert/validator';
import type { TestAnswer, Passage } from '@/lib/numericalTest/types';
...
export default function VerbalExpertResultsPage({ params }: PageProps) {
  ...
  useEffect(() => {
    ...
    const fetchSession = async () => {
      try {
        const response = await fetch('/api/verbalTestExpert/session');
        if (response.ok) {
        ...
        console.error('Error fetching verbal expert session:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const passages = (sessionId ? selectPassagesForSession(sessionId) : []) as Passage[];
    ...
    <div className="container mx-auto py-4 sm:py-6 px-3 sm:px-4 max-w-3xl">
      <div className="space-y-5 sm:space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <button
            onClick={() => router.push('/dashboard/tester/verbal-resonemang-expert')}
