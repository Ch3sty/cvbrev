...
import { ArrowRight, Lock, Layers, Clock, ListChecks, AlertCircle } from 'lucide-react';
import { useProfile } from '@/hooks/use-profile';
import { PROV_TOTAL_QUESTIONS } from '@/lib/logicTestV7/selectProv.v7';

const PROV_MINUTES = 25;

export default function LogikProvPage() {
  ...
  // Kolla om provet redan är gjort denna vecka (för gratisanvändare) genom att
  // titta på senaste prov-sessionerna. (Den skarpa spärren sker i API:t.)
  useEffect(() => {
    if (isPremium) return;
    fetch('/api/logicTestProv/session')
  ...
  const handleStart = async () => {
    setIsStarting(true);
    try {
      const res = await fetch('/api/logicTestProv/session', { method: 'POST' });
      if (res.status === 429) {
      ...
      const data = await res.json();
      if (data.session) {
        router.push(`/dashboard/tester/matrislogik-prov/test/${data.session.id}`);
      } else {
        setIsStarting(false);
      }
    } catch (error) {
      console.error('Failed to start prov:', error);
      setIsStarting(false);
    }
  };

  const nextDate = rateLimited?.nextAvailableAt
    ? new Date(rateLimited.nextAvailableAt).toLocaleDateString('sv-SE', {
        day: 'numeric',
        month: 'long',
      })
    : null;
      ...
      <div className="space-y-5 sm:space-y-6">
        ...
        >
          <svg className="absolute inset-0 w-full h-full opacity-25 pointer-events-none" aria-hidden="true">
            <pattern id="prov-hero-dots" x="0" y="0" width="24" height="24" patternUnits="userSpaceOnUse">
              <circle cx="12" cy="12" r="0.8" fill="white" />
            </pattern>
            <rect width="100%" height="100%" fill="url(#prov-hero-dots)" />
          </svg>
          <div className="relative p-6 sm:p-8 md:p-10">
            ...
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold leading-tight tracking-tight mb-3">
              Mät var du står i logik
            </h1>
            <p className="text-sm sm:text-base opacity-95 leading-relaxed max-w-xl">
              Ett skarpt prov med frågor från alla svårighetsgrader, blandade. Ingen
              förklarande hjälp under provet, precis som i ett riktigt rekryteringstest.
            </p>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h3 className="text-base font-bold text-slate-900">
                  Du har redan gjort veckans prov
                </h3>
                <p className="text-sm text-slate-600 mt-1 leading-relaxed">
                  Gratisanvändare kan göra logik-provet en gång per vecka.
