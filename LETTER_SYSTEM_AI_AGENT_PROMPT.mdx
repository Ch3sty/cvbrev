...
1. **Create New Letter**:
   ```
   1. Go to /dashboard/skapa-brev
   2. Fill in: Company "Test AB", Position "Developer"
   3. Add job description
   4. Next step
   5. Verify profile data auto-filled
   6. Toggle phone/location on/off
   7. Next step
   8. Select your new template
   9. Click "Generera brev"
   10. Wait for AI generation
   ```

2. **Verify Preview**:
   ```
   1. Check layout matches preview image
   2. Check contact info order: Name → Phone → Email → Location
   3. Check company and position appear correctly
   4. Check body text has proper paragraphs
   5. Check margins and spacing
   6. Check no HTML tags visible
   ```

3. **Test Edit Mode**:
   ```
   1. Click "Redigera"
   2. Verify textarea shows ONLY text (no HTML)
   3. Edit some text
   4. Click "Spara ändringar"
   5. Verify preview updates
   ```

4. **Test PDF Download**:
   ```
   1. Click "Ladda ned PDF"
   2. Open PDF file
   3. Check layout identical to preview
   4. Check text is selectable
   5. Check margins: 2.5cm top, 2cm sides/bottom
   6. Print preview - check page breaks
   ```

5. **Test DOCX Download**:
   ```
   1. Click "Ladda ned DOCX"
   2. Open in Microsoft Word
   3. Check NO indentation issues
   4. Check text is left-aligned
   5. Check spacing between paragraphs
   6. Check contact info complete and correct order
   7. Check no visible table borders
   8. Try editing text - should be fully editable
   ```
...
6. **Test Save & Reload**:
   ...
   3. Find your letter
   4. Click to view
   5. Verify preview looks identical
   6. Download PDF and DOCX again
   7. Verify files identical to wizard downloads
   ```

7. **Test Edit Saved Letter**:
   ```
   1. Click "Redigera" on saved letter
   2. Verify edit mode shows clean text
   3. Change text
   4. Save changes
   5. Download new PDF/DOCX
...
- Visual regression tests for PDFs
...
6. Register in both template systems
7. Add to UI selector
8. Run full test protocol
9. Document any special considerations

### 4. Debug Template Issues

When issues reported:
1. Identify which stage fails (generation, preview, download, save, edit)
2. Check relevant template code
3. Test with minimal example
4. Verify against similar working template
5. Apply fix
6. Run full test protocol
7. Verify fix doesn't break other templates

### 5. Monitor Template Quality

Regular checks:
- ✅ All templates work in wizard
- ✅ All templates work in mina-brev
- ✅ All templates work in edit mode
- ✅ PDF output matches preview
- ✅ DOCX output matches preview
...
### Template System
- `src/lib/pdf/letter-templates.ts` - HTML templates for PDF/preview
- `src/lib/letters/docx-templates.ts` - DOCX native Word templates
- `src/lib/letters/template-merger.ts` - Template data interfaces
- `src/lib/letters/extract-editable-content.ts` - Clean text extraction

### Wizard
- `src/app/dashboard/skapa-brev/page.tsx` - Main wizard coordinator
- `src/app/dashboard/skapa-brev/components/steps/BasicInfoStep.tsx` - Job info
- `src/app/dashboard/skapa-brev/components/steps/ProfileStep.tsx` - Profile data
- `src/app/dashboard/skapa-brev/components/steps/SettingsStep.tsx` - Template selection
- `src/app/dashboard/skapa-brev/components/steps/PreviewStep.tsx` - Preview & edit

### Letter Management
- `src/app/dashboard/mina-brev/page.tsx` - Letter list
- `src/app/dashboard/mina-brev/[id]/page.tsx` - View letter
- `src/app/dashboard/mina-brev/[id]/edit/page.tsx` - Edit letter

### API Routes
- `src/app/api/letters/generate/route.ts` - AI generation endpoint
- `src/app/api/letters/download/route.ts` - PDF/DOCX generation
- `src/app/api/letters/preview/route.ts` - Preview generation
...
### Utilities
- `src/lib/pdf/clean-letter-content.ts` - Content cleaning
- `src/hooks/use-letters.ts` - Letter CRUD operations
...
### Problem: PDF different from preview

**Check**:
1. @media print styles defined?
2. Puppeteer viewport correct?
3. Page margins in @page match body padding?
4. Fonts loading correctly?

### Problem: DOCX has formatting issues

**Check**:
1. All paragraphs have `indent: { firstLine: 0 }`?
2. Table cells use `hanging: 0` not `firstLine: 0`?
3. Table cells have `margins: { left: 0 }`?
4. All text uses `.trim()`?
5. All borders set to BorderStyle.NONE?

### Problem: Edit mode shows HTML

**Check**:
1. Using extractEditableContent()?
2. Setting editableText state with clean text?
3. Textarea value bound to editableText?

### Problem: Contact info missing or wrong order

**Check**:
1. Profile data passed to template?
2. Respecting include_phone_in_letters flag?
3. Respecting include_location_in_letters flag?
4. Order: Name → Phone → Email → Location?

## Your Success Metrics

You're succeeding when:
1. ✅ All 4 existing templates work flawlessly
2. ✅ New templates can be created in <1 hour
3. ✅ Template issues can be debugged in <15 minutes
