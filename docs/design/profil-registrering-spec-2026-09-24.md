...
Två delar som kan byggas och släppas var för sig. Del A rör bara inloggat läge. Del B rör /register, /login, auth-callbacken och ett nytt steg efter kontot.
...
// Renderar null när bara foto saknas eller allt är ifyllt.
// Namn saknas: StatusRow tone="warm". Annars tone="neutral".
...
|---|---|---|
| `profile_viewed` | `missing: ('namn'\|'ort'\|'foto')[]`, `anchor` (hash vid inladdning) | En gång per sidladdning. |
...
8. Statusraden: namn saknas ger varm rad, ort och foto saknas ger neutral rad, bara foto saknas ger ingen rad.
...
7. PostHog: kör `npx tsx scripts/posthog-query.ts` på `signup_flow_viewed`, `signup_intent_selected`, `signup_started`, `signup_completed`, `signup_landed` för testkontona och kontrollera att varje steg finns en gång, med rätt `intent`, `entry` och `method`.
...
1. **Mätningen lagas först (S).** `captureServer` inväntas i callbacken, Google-knappen skjuter `signup_started`. Deploya och kontrollera i PostHog att Google-konton syns. Utan det går tratten inte att läsa.
2. **Falska påståenden bort från /register (S).** Raden om fem dagar Premium och statistikrutorna, om steg 5 dröjer mer än ett par dagar.
3. **Del A, navigeringen (S).** Sidomenyn, profilmenyn, toppradens plats, PaketKort-knappen.
4. **Del A, profilsidan (M).** Sektionerna, Hoppa till, saknas-raden, fotofältet med förminskning, länkarna in.
5. **Del B, datamodell och valkommen-sidan (M).** Migrationen, `intent.ts`, cookien, `/dashboard/valkommen` med hämtkedjan (löser Google-buggen för smakproven även innan tratten finns), callbackens nya mål.
6. **Del B, /register och /login i det publika skalet (M).** Steg 1 och 2, alla lägen.
7. **Del B, steg 3 och spårvalets ändringar (S).** `ForslagSteg`, `?steg=kop`, inget förval.
8. **Ingångarna (S).** `borja` på verktygssidorna, `jc_signup_entry` i header, meny och login.
9. **QA enligt ovan, skärmdumpar, städning per id.** Sedan push.

Insats totalt: M till L. Del A kan släppas före Del B.
...
- `Smart val` spärras på `subscriptionTier`, som i dag är premium för alla tre paketen. Om ägaren vill att bara CV-paketet och Hela paketet öppnar den behövs en feature-nyckel i `features.ts`.
