...
## Visuell linje: avgjort
Byggagenten (Opus 5.5), 2026-09-23. Beslut under bygget av docs/design/analys-visuell-linje-2026-09-22.html och docs/design/analys-artiklar-2026-09-23.html, med ägarens justeringar ovan som bindande. Dirigentens ordning (tio steg) följs, inte artefaktens.

- SEO-spärren och prestandamätningen togs FÖRE steg 1, inte i steg 5: header och footer (steg 2 och 3) ändrar länkarna på varje artikel, så avtrycket måste vara från orört läge. `scripts/seo-diff-artiklar.ts` tar avtryck av topp 20 artiklar i GSC (scripts/.publikt-urval.json) plus startsidan, listan, nio verktygssidor, Funktioner, Om oss, Priser och två showcase-artiklar. Avtrycket före ligger i docs/qa/seo-diff/fore.json. Två avtryck av samma bygge ger noll skillnad (skriptet är deterministiskt).
- Diffens regler: länkar delas i innehåll och ram (publik header och footer). Ramen får ändras enligt analysen och redovisas; i innehållet på artiklar får ingen unik intern länk försvinna. På omritade publika ytor (start, verktyg, Funktioner, Om oss) får en länk flytta till ramen men inte försvinna från sidan, och sektionsrubriker får skrivas om medan h1, title, description, canonical och JSON-LD är låsta. Reklamkortens rubriker (Välj spåret du söker på, Profil, Hittade du inte vad du sökte? med flera) får lämna rubrikträdet, enligt spärrlistan punkt 3. dateModified nollas eftersom den sätts vid bygge.
- scripts/perf-publikt.ts fick `--sidor`, `--desktop`, JS över nätet med antal filer och HTML rå, så att artikelbudgeten (HTML under 150 kB, JS under 180 kB) mäts med samma skript. Före-mätningen ligger i docs/qa/qa-linje/perf-fore-pixel7.txt och perf-fore-desktop.txt. Git Bash skriver om "/" till en Windows-sökväg; kör med MSYS_NO_PATHCONV=1.
- Steg 1: text-h1, text-h1-pub, text-h2-pub, text-varde och text-tal-display ligger som klasser i globals.css i stället för fontSize i tailwind.config.js, eftersom de byter storlek vid lg och sätter familjen. text-h1 byter därmed utseende i hela inloggat läge (och i adminens två sidor som använder den) i ett slag, vilket är regel 1.
- --jc-gradient-* hade ingen användning kvar i src och togs bort i steg 1. navy och pink ligger kvar i tailwind.config.js: 15 respektive 69 filer utanför linjen (juridiska sidor, CV-analysens gamla steg, exempelpaneler) använder dem, och att ta bort dem nu skulle tappa färg där utan ersättning. Designsystemet §12 säger det.
- PageHeader: scenen visas i egen kolumn från lg och faller bort på mobil. En scen ovanför rubriken trycker vyns primära handling under första skärmhöjden på Pixel 7, vilket bryter QA-regeln om primär handling inom viewport. PageHeader fick också `eyebrow` (skisserna har "REKRYTERINGSTESTER" ovanför rubriken) och är inte längre en klientmodul.
- InkPanel pekar om --illu-fill, --illu-soft, --insunken och ink-2/3 inuti scenkolumnen, så att befintliga scener (IlluScenCv, IlluScenMatris) ritas med papper i ink-hover på bläck utan egna varianter.
- Steg 2: headern har fem val (Funktioner, Priser, Vad vi erbjuder, Artiklar, Om oss). "Hem" och "Inspiration" (/exempel) föll ur headern enligt skissen; /exempel och /cv-mallar ligger kvar i footerns Läs-kolumn så att ramen inte tappar länken. Megamenyns länkar står i HTML även stängd. Logotypens gradient byttes mot platt #F97316 (analysen: logotypens orange är det enda undantaget, och skissen ritar den platt); ordmärket i ink.
- Steg 3: footern bär samma grupper som menyn plus Läs och Om oss/Funktioner/Kontakt/Hjälpcenter/För rekryterare i första kolumnen; sociala länkar som text (Facebook, Instagram, e-post). Flaggan i bottenraden bort (hex). Cookiebannern i client-layout var en gradientknapp med orange kaka på varje publik sida; den står nu i tokens (panel, hårlinje, bläckknapp) eftersom regel 10 gäller publikt.
- Steg 4: sidomenyns Kom igång-rad var en andra bläckyta bredvid paket-huvudet och, på hemskärmen, en tredje bredvid Nästa handling. Den står nu på papper med stark kant (den flytande mobilvarianten med shadow-svav). "Profil" som egen rad föll bort (finns i profilmenyn och bottennavet), "Rapportera ett fel" står lättare än Hjälp. Låsta val läser upp "Ingår inte. Finns i ..." i aria-label i stället för att visa underraden.
- Steg 5, ramen: relaterade artiklar, verktygen och de mest lästa guiderna står under artikeln i tre kolumner, inte i sidokolumnen. Skälet: en sidokolumn som är längre än skärmen kan inte vara sticky, och innehållsförteckningens tråd är bara värd något om den följer med. Sidokolumnen (desktop) bär innehållsförteckningen och paketkortet och är sticky. De åtta guiderna ur den gamla footern står som "Mest lästa" i artikelramen (nya länkar, tillåtet).
- Steg 5, SEO: diffen mot före-avtrycket är noll fel på 37 sidor (docs/qa/seo-diff/diff-steg5.md). Författarnamnet är kvar som h3 och författarbilden kvar som första bild (samma src och alt), eftersom de fanns i avtrycket. Artiklarnas egen artikelbild renderades aldrig i sidan (bara og:image) och läggs inte till, eftersom den skulle ändra "första bild". JSON-LD rördes inte, inte ens CollectionPage-namnet med talstreck i listans schema: spärren vinner över talstrecksregeln där.
- Steg 5, showcasen (ägarens justering): mallväljaren och typsnittsvalet är kvar, som två inbyggda select-fält. Dokumentet renderas på servern av /api/public/exempel/cv och /api/public/exempel/brev (samma mallmotor som PDF:en, CDN-cachat ett dygn, noindex) och visas i en iframe som skalas med zoom. Klienten får inget mallregister och ingen mallmotor, bara id och namn för 41 mallar. Första vyn (Norrsken, Calibri) laddas med iframen, lazy.
- Steg 5, fel som hittades: klasserna text-ink-1-mjuk, border-ink-1-kant, text-ink-1-accent, text-cv, bg-cv-mjuk, text-test och bg-test-mjuk fanns inte i tailwind.config.js, bara som variabler. Prissidans Allt-kort och färgetiketter har alltså ritats utan dem sedan 22 september. Nu tillagda; prissidan får därmed de toner specen avsåg.
- Steg 5, CLS: MDX-bilderna saknade mått (width 0, height auto) och växte när de laddats; framer-animationen dolde det. CustomImage läser nu måtten ur webp-filen på servern. CLS 0 på desktop och Pixel 7.
- Steg 5, dev: artiklarna ger 500 i `next dev --turbopack` även på orörd kod (next-mdx-remote kräver react/jsx-dev-runtime utanför Next:s React). Produktionsbygget fungerar; artiklarna verifieras mot produktionsbygge.
- Steg 5, JS-budgeten (180 kB) håller inte: 244 kB i HTML-svarets skript på artikeln och 327 kB som laddas efteråt (förhämtning av länkade sidor via next/link, PostHog med inspelning). Det är rot-layoutens delar (React, Next, Supabase-klienten i AuthProvider, cookie-samtycket), inte artikeln: ramen har en klientkomponent (ArtikelKlient, under 1 kB) plus mallvisningen och mobilens klistrade knapp. framer-motion laddas bara via förhämtning av sidor som ännu inte gjorts om. scripts/js-fordelning.mjs delar upp siffran.
- Steg 6: hemskärmen i tillstånd C är display-h1 med dagens datum som eyebrow och en sammanhangsrad som bara visas när det finns ett nytt svar det senaste dygnet eller en bokad intervju (summeringen fick fältet `hem`: tysta, väntar, intervju, svar senaste dygnet, nästa intervju, räknat i samma loop som förut, ingen ny fråga). Nästa handling är sidans enda InkPanel med IlluScenUppfoljning; talet är Fordelning med segmenten tysta, väntar och intervjuer; Pågår nu är rader på mark med "Följ upp" på de tysta; aktiviteten är meningar per dag ("Det här har du gjort") ur src/lib/dashboard/aktivitet.ts, strömmad som serverkomponent i en Suspense-gräns i stället för ett klientanrop efter mount. SenasteAktivitet och klientens hämtning från /api/dashboard/recent-activity är borta (routen står kvar oanvänd). Rubriken säger "Det här har du gjort" i stället för skissens "förra veckan", eftersom meningarna grupperas per dag enligt dirigentens instruktion.
- Steg 6: toppraden säger var man är ("Träna · Rekryteringstester", "Mitt jobbsök") i stället för hälsningen, som nu är hemskärmens h1. E-postraden står kvar som rad under toppraden i skalet: den är redan en statusrad i panel, och att flytta den in under varje sidas rubrik kräver att varje sidhuvud bär den. Tillstånd B fick IlluScenBrev i egen kolumn i stället för marginalplattan; tillstånd A behåller IlluArketLyfter.
- Steg 7: testhubben har sidhuvud med eyebrow, värderubriken "Känn igen uppgiften innan provdagen." och IlluScenMatris (desktop). Rekommendationen är hubbens InkPanel: "Börja här, logiktestet på grundnivå" för den som inte gjort något, annars "Nästa test" (första test som ingår och inte är gjort), ingen bläckyta om allt är gjort. Logik, verbalt och numeriskt är paneler med en fråga i värdestil och nivåsegment (Grund, Avancerad, Expert, Prov); gjort segment har ink-kant och bästa resultat, gråa nivåer står i insunken med lås och öppnar betalväggen som förut. Personligheten står som rader direkt på mark. Sammanfattningskortet med fyra tal (TestStatsCard) och tomma-läget-kortet togs bort: det första bröt regel 5, det andra ersätts av rekommendationen; talen finns under Din utveckling, som inte rördes.
- Steg 8: CV-mallar har sidhuvud med eyebrow, värderubriken "Samma innehåll, en mall rekryteraren läser." och IlluScenMallar. Steg 1 är en StatusRow ("Steg 1 av 3 · CV: Senior Projektledare", Byt) när ett CV är valt, annars väljaren. Steg 2 är MallGrid: sex miniatyrer ur registret (den valda alltid med) och "Alla 41 mallar" som fäller ut hela galleriet, med MallMiniatyr, samma komponent som artiklarnas mallvisning. Mallar utanför paketet står i insunken med lås och går att förhandsvisa, som förut. Steg 3 är aldrig tomt: utan CV visas vald mall med exempel-CV:t via /api/public/exempel/cv. TemplateSelector och TemplateListSidebar togs bort (oanvända).
- Steg 9: startsidan följer sektionsföljden: hero med IlluScenSallet (h1-texten oförändrad, gradientordet bort), panelen "Så jobbar vi" med IlluScenBrev, mediebevisen som rader på sidans bläckyta, tre verktygskort (CV-analys, Jobbmatchning, Rekryteringstester), en lista över resten (med länkarna till CV-exempel och Personligt brev-exempel kvar i innehållet), paketen som samma PriserPaket som prissidan, och frågorna som details på mark. Kontotalet i heron läses på servern en gång per dygn (samma räkning som /api/public-stats) och faller bort om det inte går att läsa; "1 brev om dagen" och "60 sek" är produktens egna löften. FAQ-svarens priser och gratisgränser läses nu ur PLANS och kvotkonstanterna i stället för att stå som fasta tal. Nio landningskomponenter togs bort; RichFinalCTA, DynamicCounters, JobbcoachenSpotlight och JobMatchingShowcase lever kvar tills Funktioner byggs om i steg 10. Allt-kortet i prisdelen är en andra bläckyta på sidan men aldrig på samma skärm som mediebevisen.
- Steg 10: VerktygsSida (src/components/verktyg/VerktygsSida.tsx) bär alla nio verktygssidor: hero med eyebrow, oförändrad h1, ingress med fetad mening, bläckknapp och textlänk, löften i display, direkt handling (uppladdning, demo, provkort) som panel eller scen; "Så fungerar det" som numrerade rader; kontroll- och extrarader på mark; citat som panel; sidans enda InkPanel med paket och pris ur PLANS; FAQ som details. Nya scener: IlluScenLinkedin, IlluScenUpptackt, IlluScenSkapaCv (och IlluScenSkrivbordet för Om oss). Åtta av sidorna porterades av fyra parallella opus-agenter efter cv-analys som förebild, med strikt filägarskap; schemablocken är byte-identiska enligt deras diff och enligt seo-diffen. Funktioner bygger på prissidans godkända funktionstexter (FUNKTIONER i paket-copy.ts) med ankarlänkarna kvar, det som bara finns i Allt i bläck. Om oss har tidslinjen i ink med tråden på "nu" och talen (konton, brev) lästa på servern en gång per dygn.
- Steg 10: demokomponenterna som blev kvar (CV-analysens uppladdning, brevdemon, CV-byggardemon, mallväljaren, testprovkortet, coach- och matchningsdemon, LinkedIn-exemplet, Bli upptäckt-matchningen) är restylade till tokens utan framer-motion och gradient, men inte omritade från grunden.
- Fel som hittades i QA: CV-mallarnas förhandsvisning lade mallens <style> (typsnitt på body och alla element) direkt i sidan, så sidans nya display-rubrik bytte typsnitt och bröts om när förhandsvisningen kom (CLS 0,04, LCP 1,9 s). Förhandsvisningen står nu i en iframe med höjden läst ur dokumentet: CLS 0,001, LCP 1,3 s. Reservtypsnittet för display fick viktade @font-face (600, 700, 800) uppmätta mot Schibsted, eftersom next/font räknar med normal vikt.
- Prestanda, inloggat (Pixel 7, 3x CPU, LTE, ägarens eget konto, median av tre): testhubben 1 320 ms, CV-mallar 1 280 ms, alla inom 1,5 s-budgeten. Hemskärmen 1 540 ms mot budget 1 000; profilsidan, som inte rörts i omgången, mättes i samma körning till 1 552 ms. Överskridandet sitter alltså i skalet och serverns svarstid i den här miljön, inte i den nya hemskärmen; aktiviteten strömmas i en Suspense-gräns och ligger inte på den kritiska vägen. Budgeten för hemskärmen är inte uppfylld och behöver en egen prestandarunda.
- Prestanda, publikt (Pixel 7): startsidan 536 ms (före 640), logiska-tester 680 ms (före 1 520), listan 536 ms (före 1 348), cv-mall-gratis-guide 540 ms (före 1 668). HTML rå på logiska-tester 147 kB (före 594), listan 125 kB (före 154). JS över nätet 367 kB på artikeln (före 471), varav 243 kB i HTML-svarets skript. CLS 0 på alla publika vyer. Fullständiga tal i docs/qa/qa-linje/perf-*.txt.
- QA: scripts/qa-linje.mjs, 32 vyer på Pixel 7 och desktop mot produktionsbygget med två tillfälliga QA-konton (gratis och Allt med ansökningar, brev, analyser, nedladdningar och ett test), skapade med service-rollen (samma mönster som scripts/qa-paket-d2-konton.mjs; auth-användare går inte att skapa via Supabase MCP:s SQL) och raderade efteråt, kontrollerat i auth.users. Noll konsolfel, noll svar 4xx/5xx, exakt en h1 per vy, ingen horisontell scroll, startsidans primärknapp inom första skärmhöjden. Skärmdumpar i docs/qa/qa-linje/.

### saas-lead-granskning av linjen (2026-09-23, mot de två artefakterna och ägarens justeringar)
Allvarligast först.
1. Sanningskrav, Bli upptäckt: JSON-LD på /verktyg/bli-upptackt (Service, price 0) och dess FAQ säger att det är gratis att synas, men `bli_upptackt` ingår bara i Allt enligt src/lib/access/features.ts. Den synliga sidan säger nu Allt; schemat är låst av SEO-spärren. Ägarbeslut: ändra schemat (liten rankningsrisk) eller öppna synligheten gratis.
2. Sanningskrav, LinkedIn: FAQ-datan (även FAQPage-schema) nämner bara Allt, medan LinkedIn-profilen ingår i CV-veckan och slutytan säljer CV-veckan. Samma låsning, samma beslut.
3. Prestanda: hemskärmen 1,54 s mot budget 1,0 s (se ovan); orsaken ligger i skalet och delas med profilsidan. Behöver en egen runda innan ägaren bedömer "omedelbar".
4. JS-budgeten 180 kB håller inte på någon publik sida (243 kB i HTML-svaret, rot-layoutens React, Next, Supabase-klient och cookie-samtycke). Artikelramens egen klientkod är under 2 kB plus mallväljaren. Nästa steg är att ta AuthProvider och Supabase-klienten ur de publika sidornas rot, vilket är en arkitekturfråga och inte linjens.
5. Fasta priser i FAQ-data: flera verktygssidors FAQ-filer har "79 kr" och "99 kr" som text. De stämmer i dag men följer inte PLANS; de ligger i FAQPage-schema och rördes därför inte.
6. Avvikelse mot artefakten: artikelbilden 1200 × 630 ritas inte i artikeln (den gjorde det inte före heller; bara og:image). Att lägga till den skulle ändra "första bild" i spärren. Relaterade artiklar, verktyg och mest lästa står under artikeln i stället för i sidokolumnen, så att innehållsförteckningen kan vara sticky.
7. Avvikelse: e-postraden står kvar i skalet ovanför innehållet i stället för som statusrad under varje rubrik.
...
## Räkna ut: avgjort
Byggagenten (Opus 5.5), 2026-09-23, gren `linje/rakna-ut`. Del 1 (två småfixar) och del 2 (Räkna ut-hubben och de nio kalkylatorerna i linjen, delbart resultat). Commits 42aed68b (del 1), 0589f11b (baslinje före), e2ebafac (del 2).

### Del 1
- `/verktyg/linkedin-optimering`: WebApplication- och HowTo-schemat sa "Karriärguidens AI", "Split-view", "branschkeywords", "score-rapport" och "copy-paste"; FAQ-frågan hette "Vad är ATS-keywords på LinkedIn?". Nu Jobbcoach.ai, "rekryteringssystem (ATS)", "nyckelord", "delad vy", och frågan "Vilka nyckelord letar rekryteringssystemen (ATS) efter på LinkedIn?". h1, title, description, canonical orörda. Posten i BASLINJE_OMTAGEN fanns redan (ägarbeslutet om paket), skälet förlängdes i stället för en andra post, eftersom skriptet slår upp första träffen per sökväg. Baslinjen omtagen med `baslinje --sidor /verktyg/linkedin-optimering`.
- `vad-ar-en-jobbcoach`: frontmatterns FAQ och de synliga FAQItem för "Ersätter Jobbcoach.ai..." och "Vad kostar Jobbcoach.ai?" har dagens paket ur PLANS (CV-veckan 79, Testveckan 79, Allt-veckan 99, Allt-månaden 149) och gratisnivån (3 mallar, en CV-analys med poäng och tyngsta fyndet, ett brev var sjunde dag att läsa, grundnivån i testerna). **Avgjort:** fyra rader i brödtexten sa också "Premium 149 kr/mån" (två verktygsrader, pristabellen, avslutningen). De rättades i samma commit eftersom de annars motsäger FAQ:n. Sidan ingår inte i seo-diffen (inte bland topp 20), därför ingen post i BASLINJE_OMTAGEN.

### Del 2, avgjort
1. **Syskonmall, inte VerktygsSida.** Kalkylatorerna behöver fält och resultat bredvid varandra direkt under rubriken och har ingen "Så fungerar det"-lista eller citat, så de fick `src/components/rakna/KalkylatorSida.tsx`: hero med scen (från lg), kalkylatorn, "Så räknar vi" med källorna som lista bredvid, nästa steg som upphöjd panel, frågorna som details direkt på mark (inte paneler i följd, regel 4), Läs mer som lista. RaknaUtSida är borttagen. Schemat (WebApplication, FAQPage) och brödsmulorna byggs exakt som förut.
2. **Resultatet är sidans enda bläckyta.** InkPanel med kalkylatorns namn som etikett, förutsättningarna som rubrik ("42 000 kr i lön i Uppsala"), talet i `text-tal-display` och Fordelning där det finns en fördelning (lön efter skatt: netto/skatt, vad kostar en anställd: lön/avgift/avtal, felrekrytering: fyra poster, semesterersättning: dagslön/tillägg). Därför säljer kalkylatorsidorna paketet i en upphöjd panel, inte i bläck: lönesidorna Allt med jobbcoachen, rekryterarsidorna insikterna för rekryterare utan paket. Hubben har bläckytan (löneförhandlingens värde plus jobbcoachen i Allt).
3. **Fordelning fick två additiva props:** `total: number | string` och `visa?: string` per segment, så att kronbelopp står formaterade ("26 912 kr"). Dessutom `flex-wrap` och `whitespace-nowrap` på talet, som annars bröts på 412 px. Dashboardens användning påverkas inte. På bläck pekas tonerna om lokalt med `PA_BLACK` i `src/components/rakna/ui.tsx` (samma grepp som InkPanel gör för scenen).
4. **Logiken flyttades oförändrad till `src/lib/rakna/`**, en modul per kalkylator med `las`, `parametrar`, `berakna` och `sammanfatta`. MDX-kalkylatorerna i insikterna (Felrekrytering, Sourcing, Träffsäkerhet) behåller sitt utseende men importerar räkningen därifrån. Enda textändring i logikfilerna: kanalen "Kandidatpool med opt-in" heter "Kandidatpool med samtycke" (svenska facktermer), vilket också syns i insiktens MDX-kalkylator.
5. **Delningslänken.** Varje kalkylator skriver sitt läge till query-strängen (som förut via `useQuerySynk`, nu även semesterersättning, timlön, uppsägningstid, sourcing och träffsäkerhet). Befintliga parameternamn behölls, så äldre delade länkar fungerar. Varje kalkylator har en nyckelparameter som alltid står med (`NYCKEL` i `src/lib/rakna/delning.ts`: lon, vem, lage, belopp, kanal, metod).
6. **Delad vy via rewrite, statisk sida kvar.** OG-bilden per resultat kräver att servern ser parametrarna, men de statiska sidorna ska ligga kvar i ISR (revalidate 86400) för Vercel-kostnaden. Därför skriver `next.config.ts` (beforeFiles, `has: query`) om `/rakna-ut/<slug>?<nyckel>=...` till `/rakna-ut/<slug>/delad`, en dynamisk sida som räknar resultatet på servern (rätt tal i HTML före hydrering) och sätter og:image/twitter:image till `/api/og/rakna-ut/<slug>?...` och og:title till resultatet. Title, description och canonical är den vanliga sidans. Utan parametrar: oförändrad statisk sida med `images/rakna-ut.webp`. Ett test håller NYCKEL och rewrites i takt.
7. **OG-routen** `src/app/api/og/rakna-ut/[slug]/route.tsx` (node-runtime) ritar kalkylatorns namn, förutsättningarna, talet i Schibsted Grotesk 800, en orange linje under, enheten, en räknare och jobbcoach.ai-märket i Trådens toner, källan nere till höger. Typsnitten ligger i `src/assets/fonts` (Schibsted Grotesk 500 och 800, 95 kB var) och tas med via `outputFileTracingIncludes`. Tokenvärdena står som hex i `bild.tsx` eftersom Satori inte läser CSS-variabler; det är det enda stället. Cache-Control `s-maxage=31536000, immutable`, och proxyn hoppar över `/api/og/` (matcher).
8. **Dela-knappen** (`DelaResultat.tsx`): med `navigator.share` och grov pekare öppnas delningsmenyn, annars kopieras länken ("Länken är kopierad", status i aria-live); går det inte visas länken markerad i ett fält. Bädda in finns kvar som textlänk på bläckytan och fäller ut koden under resultatet.
9. **Prestanda.** Skattetabellerna (122 kB) laddas med dynamisk import efter första renderingen; servern räknar startresultatet och skickar med det. En gemensam serverväljare som importerade alla nio klientkalkylatorer gav 180 kB JS på varje sida, så varje sida har en egen startfil i `src/components/rakna/start/` (Kalkylator.tsx med alla nio används bara av /embed). Uppsägningstiden får dagens datum från servern vid första renderingen och byter till webbläsarens direkt efter, så att HTML och hydrering är överens.
10. **Mobil:** "Träffsäkerhetssimulatorn:" rymdes inte i 32 px på 412 px och gav vågrät rullning. h1 får 28 px under sm när titeln har ett ord över 18 tecken, plus `min-w-0` och `overflow-wrap:anywhere`. Scenen står bara från lg på kalkylatorsidorna, så att fälten syns i första skärmen på mobil; på hubben står den under heron som på verktygssidorna.
11. **Copy:** korten har värde före funktion ("Räkna fram din sista anställningsdag" över "Uppsägningstid enligt LAS"); korttexterna och alla h1, ingresser, metodtexter och FAQ är ordagrant som förut. Priser ur PLAN_BY_KEY, gratismeddelanden ur FREE_CHAT_MESSAGES_PER_ACCOUNT. Inga talstreck, ingen Sparkles; grep-kontrollen i designsystemet §10 ger noll träffar i de nya filerna (dela.tsx, som bara MDX-kalkylatorerna använder, står kvar i gammal stil).

### Mätt
- **seo-diff** (`docs/qa/seo-diff/diff-rakna-ut.md`): de tio Räkna ut-sidorna plus LinkedIn, 0 fel. h1, title, description, canonical, og:image, robots och JSON-LD oförändrade; sektionsrubriker ändrade (tillåtet på omritade ytor); inga länkar borta, nya länkar till /verktyg/jobbcoachen, /priser och /for-rekryterare. De tio sidorna ligger nu i OVRIGA och i fore.json.
- **LCP** Pixel 7, 3x CPU, LTE, median av tre (`docs/qa/qa-rakna-ut/perf-fore.txt`, `perf-efter.txt`): hubben 488 → 560 ms, lön efter skatt 552 → 596, felrekrytering 548 → 560, löneförhandling 728 → 716, semesterersättning 596 → 564, sourcing 544 → 472, timlön 556 → 608, träffsäkerhet 556 → 504, uppsägningstid 672 → 636, vad kostar en anställd 556 → 492, delad vy (lön efter skatt med parametrar) 496. Alla under 1 000 ms, CLS 0 överallt.
- **JS i HTML-svaret** (`js-fore.txt`, `js-efter.txt`): 158 till 168 kB på alla elva, under 180. Lön efter skatt 205 → 168 kB.
- **tsc** rent, **vitest** 714 av 714 i 56 filer (nya: `src/lib/rakna/__tests__/delning.test.ts` 53 tester för parametrarnas väg genom länken, nyckeln, rewrites och räkningens förval; `src/app/api/og/rakna-ut/[slug]/__tests__/route.test.ts` 4 tester för PNG 1200 × 630 per kalkylator, 404 och talstorleken). **Produktionsbygge** rent (`NEXT_DIST_DIR=.next-rakna`, port 5216). Bygget kräver `NEXT_TURBOPACK_EXPERIMENTAL_USE_SYSTEM_TLS_CERTS=1` på den här maskinen (next/font fick TLS-fel mot Google Fonts), och Google Fonts svarade ibland inte alls; ett nytt försök räckte.
- **Webbläsartest** (`scripts/qa-rakna-ut.mjs`, `docs/qa/qa-rakna-ut/`): hubben och alla nio på Pixel 7 och desktop, ett räkneexempel ifyllt per kalkylator, Dela tryckt (Pixel 7: navigator.share ersatt med en stubbe som sparar vad som delades; desktop: länken ur urklipp), den delade länken öppnad i en ny flik med samma tal och rubrik som originalet, talet redan i serverns HTML, OG-bilden hämtad (200, image/png) och sparad som `og-<slug>.png`. Inga konsolfel, inga 4xx/5xx, ingen vågrät rullning. Skärmdumpar per steg: `<enhet>-<slug>-1-forsta-vy`, `-2-ifylld`, `-3-hela`, `-4-delad`, `-5-ny-flik`; före-bilder `fore-*`. Sammanställning i `qa-rapport.json`.

### Kvar och att veta
- Den riktiga delningsmenyn på en telefon är inte klickad (headless kan inte öppna den); det som testats är att knappen anropar Web Share med titel, text och länk.
- Delade vyer renderas per förfrågan (ƒ). OG-bilderna cachas ett år i CDN:et per parameteruppsättning.
- Årsrevisionen i januari (project_rakna_ut_hubb) gäller nu `src/lib/rakna/*` och `src/data/*`; OG-routen och sidorna läser samma moduler.
- Minne att uppdatera: project_rakna_ut_hubb (komponenterna bor i src/lib/rakna och src/components/rakna/start, delad vy och OG-route).
