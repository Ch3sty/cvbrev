...
async function analyzeCV(cvText: string, targetInfo: any, openaiApiKey: string, openaiProjectId?: string) {
  ...
  console.log(`Using project: ${openaiProjectId ? 'cvbrev (proj_WlM3ZDwbSPysgXRdp4m8dEGg)' : 'default'}`);
  ...
  try {
    ...
    console.log(`Headers: Authorization: Bearer sk-...${openaiApiKey?.slice(-4)}, OpenAI-Project: ${openaiProjectId || 'not set'}`);
