...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Hämta aktivt CV från active_cv_for_matching
    ...
    // Extrahera parsed data från aktivt CV
    const cvOccupations = (activeCV.extracted_occupations || []).map((o: any) => o.normalized);
    const cvLocation = activeCV.extracted_location;
    const cvLocations = cvLocation ? [cvLocation] : [];

    // Prioritera high-confidence occupation
    const primaryOccupation = (activeCV.extracted_occupations || []).find((o: any) => o.confidence === 'high') || activeCV.extracted_occupations?.[0];

    if (!primaryOccupation) {
      return new Response(JSON.stringify({
        error: 'Inga yrkesroller hittades i ditt CV.',
        code: 'NO_OCCUPATIONS'
      }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }

    console.log('[Match-Jobs] Primary occupation:', primaryOccupation.normalized, '(concept_id:', primaryOccupation.concept_id, ')');
    console.log('[Match-Jobs] All occupations:', cvOccupations);
    console.log('[Match-Jobs] Location:', cvLocation);

    // Vi har redan normaliserad data från parse-cv-for-matching, så vi behöver inte hämta taxonomy igen
    let taxonomyData = {
