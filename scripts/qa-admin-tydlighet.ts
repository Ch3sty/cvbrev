/**
 * QA av adminomgången "Admin utan streck" (docs/design/spec-admin-tydlighet-2026-09-22.html).
 * Bygger på scripts/qa-admin.ts och lägger till tre innehållskontroller per vy:
 * inga streck som värde, inga em-dash, och ägarens e-post syns bara där
 * undantagen ska synas (Användare, filtret Admin och test, och Inställningar).
 * Köpet 22 sep (Allt-dagen 49 kr) ska synas på Översikt och Intäkter.
 *
 *   npx tsx scripts/qa-admin-tydlighet.ts --port 5211
 *
 * Ursprunglig beskrivning:
 * QA av hela adminen (docs/plan-admin.md, våg 4).
 *
 * Skapar ett engångskonto, ger det super_admin i admin_users, och går igenom
 * alla nio sidorna plus Användare/[id], Innehålls fem flikar och Funnel 26v.
 * Skärmdump per vy på desktop 1280 och Pixel 7, LCP och CLS som median av tre,
 * och en kontroll av konsolfel, 4xx/5xx, horisontell scroll, sidfot och
 * cookie-banner. Kontot raderas alltid till sist, även vid fel.
 *
 ...
 *
 * Om skärmdumpen: AdminShell är h-dvh med scrollen i en inre main.overflow-y-auto,
 * så fullPage ger bara den första skärmen. Vi mäter det elementets scrollHeight
 * och sätter viewporthöjden till det i stället.
 */
...
const UT = 'docs/qa/qa-admin-tydlighet';

/** Ägarens e-post. Får bara synas i vyerna nedan. */
const AGARE = 'christiankarlssson';
const AGARE_TILLATEN = new Set(['anvandare-admin-test', 'installningar', 'anvandare-agare']);

/** Vyer där köpet 22 sep måste synas. */
const KOP_SYNS = new Set(['oversikt', 'intakter', 'anvandare-detalj', 'tratt-kopvag']);
...
const VYER: Vy[] = [
  { namn: 'oversikt', path: '/admin', titel: 'Översikt' },
  { namn: 'intakter', path: '/admin/intakter', titel: 'Intäkter' },
  ...
  { namn: 'tratt-anvandning', path: '/admin/tratt?vy=anvandning', titel: 'Tratt: Användning' },
  { namn: 'mejl', path: '/admin/mejl', titel: 'Mejl' },
  { namn: 'drift', path: '/admin/drift', titel: 'Drift' },
  { namn: 'innehall-cv', path: '/admin/innehall?flik=cv', titel: 'Innehåll: CV' },
  { namn: 'innehall-brev', path: '/admin/innehall?flik=brev', titel: 'Innehåll: Brev' },
  { namn: 'innehall-mallar', path: '/admin/innehall?flik=mallar', titel: 'Innehåll: Mallar' },
  { namn: 'innehall-kandidater', path: '/admin/innehall?flik=kandidater', titel: 'Innehåll: Kandidater' },
  { namn: 'innehall-rekryterare', path: '/admin/innehall?flik=rekryterare', titel: 'Innehåll: Rekryterare' },
  { namn: 'installningar', path: '/admin/installningar', titel: 'Inställningar' },
];
...
async function main() {
  ...
  try {
    ...
    // Detaljsidan visar kontot som köpte Allt-dagen 22 sep, så att sektionen
    // Köp syns med ett riktigt köp.
    const detaljId = '580ee411-0ffb-4fd2-8a41-7216750c7c39';
    ...
    for (const vy of VYER) {
      ...
      for (let i = 0; i < KORNINGAR; i++) {
        ...
        try {
          ...
          const innehall = await page.evaluate(() => {
            const main = document.querySelector('main') ?? document.body;
            const text = (main as HTMLElement).innerText;
            // Ett streck som ensamt värde i ett element: det spec-principen förbjuder.
            const streck = Array.from(main.querySelectorAll('*'))
              .filter((e) => e.children.length === 0 && /^[–—-]$/.test((e.textContent ?? '').trim()))
              .map((e) => (e.parentElement?.textContent ?? '').trim().slice(0, 60));
            return { text, streck, emdash: text.includes('—') };
          });
          ...
          if (i === KORNINGAR - 1) {
            if (innehall.streck.length) problem.push(`${vy.titel}: ${innehall.streck.length} streck som värde: ${innehall.streck.slice(0, 3).join(' | ')}`);
            if (innehall.emdash) problem.push(`${vy.titel}: em-dash i texten.`);
            if (innehall.text.includes(AGARE) && !AGARE_TILLATEN.has(vy.namn)) problem.push(`${vy.titel}: ägarens e-post syns.`);
            if (KOP_SYNS.has(vy.namn) && !(innehall.text.includes('Allt-dagen') && innehall.text.includes('49 kr'))) problem.push(`${vy.titel}: köpet 22 sep (Allt-dagen 49 kr) syns inte.`);
            console.log(`  streck ${innehall.streck.length} | em-dash ${innehall.emdash ? 'JA' : 'nej'} | Allt-dagen 49 kr ${innehall.text.includes('Allt-dagen') && innehall.text.includes('49 kr') ? 'ja' : 'nej'}`);
            sista = {
              vy: vy.namn,
              titel: vy.titel,
              lcp: 0,
    ...
    // ------------------------------------- gamla adresser till Tratt
    for (const [gammal, mal] of [['/admin/flode', '/admin/tratt'], ['/admin/funnel', '/admin/tratt?vy=veckor']]) {
      const s = await nySida('desktop', epost);
      try {
        await s.goto(bas + gammal, { waitUntil: 'networkidle0', timeout: 60000 });
        const slut = s.url().replace(bas, '');
        console.log(`  ${gammal} -> ${slut}`);
        if (!slut.startsWith(mal)) problem.push(`${gammal} skickas inte till ${mal} (hamnade på ${slut}).`);
      } finally {
        await s.close();
      }
    }
