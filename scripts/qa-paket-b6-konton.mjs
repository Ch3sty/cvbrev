...
// Skapar och raderar QA-kontot för klicktestet av ångerrättssamtycket (B6).
//
...
// vänder. Kontot skrivs till /tmp/qa-b6.json och raderas i samma omgång.
...
const FIL = '/tmp/qa-b6.json'
const LOSEN = 'QaPaketB6!2026'

async function skapa() {
  const email = `qa-b6-${Date.now()}@jobbcoach-qa.test`
  const { data, error } = await admin.auth.admin.createUser({
    email,
    password: LOSEN,
    email_confirm: true,
    user_metadata: { full_name: 'QA Samtycke B6' },
  })
  ...
  await admin.from('profiles').upsert({ id: userId, email, full_name: 'QA Samtycke B6' })
